            ...
            if(parseFloat(data.amount) <= parseFloat(userbal)){
              if(data.luckygen==0)
              {
                ...
                if(calculatedAmount<=data.amount && calculatedAmount>0){
                  ...
                  console.log("normal");
                  if(dudo_win){
                      var str = dudo_win.loss_count;
                      var split_str = str.split(",");
                      var bet_cnt = dudo_win.betcount.toString();
                      ...
                      if(loss_create==1){
                        if(data.rollStatus == 'over'){
                        var luck_data = {min:dicesett.min_range, max:data.prediction, type:'loss'};
                        res.json({ status: true, msg:"lucky",data:luck_data });  
                        
                        }else{
                            var luck_data = {min:data.prediction, max:dicesett.max_range, type:'loss'};
                            res.json({ status: true, msg:"lucky",data:luck_data });
                           
                        }
                    }else{
                      if(data.rollStatus=='over'){
                        var luck_data = {min:data.prediction, max:dicesett.max_range, type:'win'};
                        res.json({ status: true, msg:"lucky",data:luck_data });
                      }else{
                         var luck_data = {min:1, max:data.prediction, type:'win'};
                         res.json({ status: true, msg:"lucky",data:luck_data });
                      }
                  ...
                  var dudo = await dudowin.findOne({userId:data.user_id,prediction:winningChance}).lean();
                  ...
                  if(dicewin_sta){
                    if(dicewin_sta.betcount==100){
                      ...
                      let dicewin_create = await dicewin.create({userId:data.user_id,prediction:winningChance,loss_count:dicewin_rand,betcount:1});
                    }
                  ...
                  var dicewin_sta = await dicewin.findOne({userId:data.user_id,prediction:winningChance}).lean();
                  var dudowin_sta = await dudowin.findOne({userId:data.user_id,prediction:winningChance}).lean();
                  if(dicewin_sta){
                       ...
                       if(split_str.indexOf(bet_cnt) !== -1){
                          ...
                          var str = dudowin_sta.loss_count;
                          var split_str = str.split(",");
                          var bet_cnt = dudowin_sta.betcount.toString();
                          if (split_str.indexOf(bet_cnt) !== -1) {
                              console.log("loss");
                              if(data.rollStatus == 'over'){
                              var luck_data = {min:dicesett.min_range, max:data.prediction, type:'loss'};
...
async function savedicedata(data,winStatus,multiplier,profit,profitToWin,userbal,winningChance) {
  ...
  var divisett = await diviSettings.findOne();
  var dudowin_sta = await dudowin.findOne({userId:data.user_id,prediction:winningChance}).lean();
  var random = await table_rand(winningChance);
  if(dudowin_sta){
    if(dudowin_sta.betcount==100){
      let dudowin_remove = await dudowin.remove({userId:data.user_id,prediction:winningChance});
      let dudowin_create = await dudowin.create({userId:data.user_id,prediction:winningChance,loss_count:random});
    }else{
      let dudowin_upd = parseInt(dudowin_sta.betcount)+1;
      let dicewin_chg = await dudowin.updateOne({userId:data.user_id,prediction:winningChance},{$set:{betcount:dudowin_upd}});
    }

...
redClient.on('connect', () => {
    console.log(`Redis client [connected] is running on ${config.redisData.host}:${config.redisData.port}/${config.redisData.db}`);
});
...
const getFrequentWithdrawal = async (user) => {
    await redisIsOk();

    const fw = await redClient.get(`fw_${user}`);
    return !!fw;
};

const setFrequentWithdrawal = async (user) => {
    await redisIsOk();

    await redClient.set(`fw_${user}`, 'true', 'EX', config.withdrawFrequencySeconds);
...
const setBongoRequest = async (request) => {
    ...
    await redClient.set(`bongo_${request.uid}`, JSON.stringify(request), 'EX', config.bongoRequestExpireSeconds);
    return true;
};
